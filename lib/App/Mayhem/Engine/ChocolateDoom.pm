package App::Mayhem::Engine::ChocolateDoom;
...
App::Mayhem::Engines::ChocolateDoom - A module that wraps the C<Chocolate
...
The B<Chocolate Doom> Doom engine, an engine for the game B<Doom>.
    ...
    use App::Mayhem::Engine::ChocolateDoom;
...
Returns a L<App::Mayhem::Engine::ChocolateDoom> object.  Use the
...
sub BUILD {
    ...
    foreach my $path ( @search_paths ) {
        if ( -x $path . q(/) . $self->binary_name() ) {
            ...
            $log->debug(qq(Choco: found chocolate-doom binary at:)
                . $self->binary_path() );
        } # if ( -x $path . q(/) . $self->binary_name() )
    } # foreach my $path
    return $self;
...
sub engine_description {
    return <<DESC
Chocolate Doom is a Doom source port developed by Simon "Fraggle" Howard. Its
name is a joke around the phrase Vanilla Doom. Unlike other ports which attempt
to fix the bugs in the original engine and add new features, Chocolate Doom
deliberately attempts to behave as identically as possible to the original
Vanilla Doom.
DESC
}
...
sub engine_homepage_uri {
    return q(http://www.chocolate-doom.org);
}
...
1; # End of App::Mayhem::Engines::ChocolateDoom
