...
=head1 TEST: App::Mayhem::View object
...
diag( qq(\nTesting App::Mayhem::View )
    . qq($App::Mayhem::View::VERSION,\n)
    . qq(Perl $], $^X\n)
...
### TEST GUTS ###
    ...
    ## TEST
    ok($test_thread->is_detached(), q(View thread is detached));
    ...
    while (1) {
        ...
        while ( $client = $server->accept() ) {
            ...
            my ($command, $remote_name, $api_ver_string, $remote_api_ver)
                                        = split(/ /, $init_message);
            ok($init_message =~ /HELLO View api_version \d+/,
                qq(View sent hello message: '$init_message'));
